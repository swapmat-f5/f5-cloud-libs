 ...
 * @param {String}  [options.bigIpMgmtAddress] - IP address of BIG-IP management port. Default is that returned by device info.
 * @param {String}  [options.bigIpMgmtPort]    - Port of BIG-IP management port. Default is this.core.port
 *
 * @returns {Promise} A promise which is resolved when the licensing
 *                    is complete or rejected if an error occurs.
 */
BigIpOnboard.prototype.licenseViaBigIq = function(host, user, password, poolName, options) {
    ...
    return this.core.ready()
        ...
        .then(function(response) {
            ...
            if (util.versionCompare(version, '5.0.0') >= 0 && util.versionCompare(version, '5.2.0') < 0) {
                licenseProvider = new BigIq5_0LicenseProvider(this.core, this.options);
            }
            else if (util.versionCompare(version, '5.2.0') >= 0) {
                licenseProvider = new BigIq5_2LicenseProvider(this.core, this.options);
            }

            // Get our management IP if it was not given to us
            if (!options.bigIpMgmtAddress) {
        ...
        .then(function(response) {
            ...
            var bigIpMgmtAddress;

            if (response || options.bigIpMgmtAddress) {
                bigIpMgmtAddress = options.bigIpMgmtAddress || response.managementAddress;
            }
            ...
            return licenseProvider.getUnmanagedDeviceLicense(
                ...
                poolName,
                bigIpMgmtAddress,
                options.bigIpMgmtPort || this.core.port
            );
        }.bind(this))
        .catch(function(err) {
            return q.reject(err);
