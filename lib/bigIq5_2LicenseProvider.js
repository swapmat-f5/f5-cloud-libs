    ...
    return bigIqControl.list(LICENSE_PATH + '?$select=id,name')
        ...
        .then(function(response) {
            ...
            var findValidLicense = function(index) {
                ...
                if (new Date(license.licenseState.licenseStartDateTime) < now && now < new Date(license.licenseState.licenseEndDateTime)) {
            ...
            var isLicensed = function() {
                return bigIqControl.list(LICENSE_PATH + poolUuid + '/offerings/' + regKey + '/members/' + memberId)
                    .then(function(response) {
                        ...
                        this.logger.verbose('Current licensing status:', status);
                        if (status === 'LICENSED') {
                            return q();
                        }
                        else {
